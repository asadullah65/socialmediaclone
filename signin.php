...
if (isset($_POST['login'])) {
    if (isset($_POST['user_login']) && isset($_POST['password_login'])) {
        $user_login = $connection->real_escape_string($_POST['user_login']);
        $user_login = mb_convert_case($user_login, MB_CASE_LOWER, "UTF-8");    
        $password_login = $connection->real_escape_string($_POST['password_login']);
        $rememberme = $_POST['rememberme'];        
        $password_login_md5 = md5($password_login);
        
        $query = "SELECT * FROM users WHERE (username='$user_login' OR email='$user_login') AND password='$password_login_md5' AND activated='1' AND blocked_user='0'";
        ...
        if ($num > 0) {
            ...
            $query1 = "SELECT * FROM users WHERE (username='$user_login' OR email='$user_login') AND password='$password_login_md5' AND activated='0'";
            $result1 = $connection->query($query1);
            $num1 = $result1->num_rows;
            $get_user_email = $result1->fetch_assoc();
            ...
            if ($num1 > 0) {
                ...
                $success_message = '
                    ...
                    <font face="bookman">Account activation code sent to you. <br>
                        Please check your mail: '.$get_user_email_db.'
                    </font>
                    <form action="signin.php" method="POST">
                        Enter verification code:<input type="text" name="confrmCode" class="submRecov" size="30" required></br>
                        <input class="submRecov" type="submit" name="submconfrmCode" id="senddata" value="Continue Daowat">
                    </form>
                    </div>';
            } else {
                $query2 = "SELECT * FROM users WHERE (username='$user_login' OR email='$user_login') AND password='$password_login_md5' AND blocked_user='1'";
                $result2 = $conn->query($query2);
                $num2 = $result2->num_rows;
                
                if ($num2 >= 1) {
                    $success_message = '
                    <h2><font face="bookman">Oops!!!</font></h2>
                        <div class="maincontent_text" style="text-align: center;">
...
if (isset($_POST['submconfrmCode'])) {
    $user_confrmCode_db = $connection->real_escape_string($_POST['confrmCode']);
    $user_loginnn = $_SESSION['user_loginn'];
    $query3 = "SELECT * FROM users WHERE username='$user_loginnn' AND confirmCode='$user_confrmCode_db' AND activated='0'";
    $result3 = $connection->query($query3);
    $num2 = $result3->num_rows;
    
    if ($num2 >= 1) {
        $query_update = "UPDATE users SET activated='1', confirmCode='0' WHERE username='$user_loginnn'";
        ...
        $success_message = '
            <div class="maincontent_text" style="text-align: center;">
            <font face="bookman">Incorrect verification code
            </font>
            <form action="signin.php" method="POST">
                Enter verification code:<input type="text" name="confrmCode" class="submRecov" size="30" required></br>
                <input class="submRecov" type="submit" name="submconfrmCode" id="senddata" value="Continue Daowat">
...
$sql_uname_check = mysqli_query($connection, "SELECT id FROM users WHERE username='$username' LIMIT 1");
if ($sql_uname_check) {
...
$_POST['username'] = preg_replace('/\s+/','',$_POST['username']);
	try {
		...
		// Username check
$u_name = $connection->real_escape_string($_POST['username']);
$u_check = $connection->query("SELECT username FROM users WHERE username='$u_name'");
$check = $u_check->num_rows;

// Check if email already exists
$u_email = $connection->real_escape_string($_POST['email']);
$e_check = $connection->query("SELECT email FROM users WHERE email='$u_email'");
    ...
    if ($check == 0) {
        if ($email_check == 0) {
            if (strlen($_POST['password']) > 4) {
                ...
                $stmt = $connection->prepare("INSERT INTO users (first_name, username, email, password, gender, sign_up_date, activated) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $activated = 1;
                $stmt->bind_param("ssssssi", $_POST['first_name'], $_POST['username'], $_POST['email'], $_POST['password'], $_POST['gender'], $d, $activated);
                $stmt->execute();
                $stmt->close();
                ...
                // $user_to = 'nur';
                // $create_followMe = $conn->query("INSERT INTO follow VALUES ('', '$user_from', '$user_to', NOW(), 'no')");
                // $create_followFrom = $conn->query("INSERT INTO follow VALUES ('', '$user_to', '$user_from', NOW(), 'no')");

                // Send message
                // $msg_body = 'Assalamu Alaikum';
                // $msgdate = date("Y-m-d");
                // $opened = "no";
                // $messages = $conn->query("INSERT INTO pvt_messages VALUES ('', '$user_to', '$user_from', '$msg_body', '$msgdate', NOW(), '$opened', '')");
...
$query = "SELECT * FROM daowat WHERE photos != '' ORDER BY RAND()";
...
// Optional: Close the connection if it's no longer needed
// $connection->close();
